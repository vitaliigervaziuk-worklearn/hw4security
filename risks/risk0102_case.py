...
""")
    ...
    # block rate and escape rate across benign and malicious prompts.
    st.markdown("---")
    st.subheader("Security Evaluation Metrics")
    st.caption("Probabilistic evaluation over a labeled test suite: 5 benign + 6 malicious prompts.")
    ...
    if st.button("Run Injection Evaluation", key="pi_eval"):
        ...
        col_b.metric("False Positive Rate", f"{metrics['false_positive_rate']:.0%}",
                     help="Fraction of benign prompts wrongly blocked by the guard.")
        ...
        st.caption(
            ...
            f"{metrics['total'] - metrics['total_attacks']} benign)."
        )
